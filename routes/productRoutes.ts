...
interface ProductWithScans extends Product {
     scans: Scan[];
  }
...
const getPrismaClient = (databaseUrl: string): PrismaClient => {
     if (!prismaClient) {
...
const fetchLocationName = async (latitude: number, longitude: number): Promise<string> => {
 ...
 try {
  ...
  qrCodeRouter.post('/sign-batch', async (c) => {
       try {
           const { name, stationId, numberOfCodes } = await c.req.json<QRCodeRequestData>();
  ...
  qrCodeRouter.post('/scan', async (c) => {
        try {
             const { signedQRCode, location } = await c.req.json<ScanRequestData>();
             if (!signedQRCode || !location?.latitude || !location?.longitude) {
                   return c.json({ error: 'Invalid input parameters' }, 400);
                }
    
            const decodedData = await verify(signedQRCode, c.env.AES_SECRET_KEY) as DecodedQRData;
             const prisma = getPrismaClient(c.env.DATABASE_URL);
                  ...
                  data: {
                     batchId: masterQR.batchId,
                      createdAt: masterQR.createdAt,
                        products: products.map((product: ProductWithScans) => ({
                              ...product,
                             scans: product.scans.map((scan: Scan) => ({
                                    location: {
